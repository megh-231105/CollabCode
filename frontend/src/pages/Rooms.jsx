...
const Rooms = () => {
  ...
  const handleCopyId = (id) => {
    ...
    setCopiedId(id);
    showToast(`Room ID ${id} copied! Share with peers to join.`);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              className="uiverse-btn-glow flex items-center space-x-2 px-4 py-2.5 bg-gradient-to-r from-emerald-400 via-teal-300 to-emerald-400 hover:from-emerald-300 hover:to-teal-200 text-slate-950 font-black rounded-xl text-xs transition shadow-lg shadow-emerald-500/20 cursor-pointer"
            >
            ...
            <button
              ...
              className="flex items-center space-x-2 px-4 py-2.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-cyan-500/40 text-cyan-300 font-bold rounded-xl text-xs transition cursor-pointer"
            >
        ...
        {filteredRooms.length > 0 ? (
              ...
              >
                <div>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      ...
                      className="text-xs font-mono font-bold bg-slate-950 hover:bg-slate-800 px-2.5 py-1 rounded-lg border border-slate-800 hover:border-cyan-500/40 text-cyan-300 flex items-center space-x-1.5 transition cursor-pointer"
                    >
                  ...
                  <p className="text-xs text-slate-400 line-clamp-2 mb-4 leading-relaxed">
                    {room.description || 'Collaborative coding workspace session on MongoDB Atlas.'}
                  </p>
                </div>
                ...
                <div className="pt-4 border-t border-slate-800/80 space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    <Link
                      to={`/rooms/${room.id}`}
                      className="flex-1 flex items-center justify-center space-x-2 py-2.5 bg-slate-800 hover:bg-emerald-400 hover:text-slate-950 text-slate-200 border border-slate-700 hover:border-emerald-400 font-bold rounded-xl text-xs transition duration-150"
                    >
                    ...
                    <button
                      ...
                      title="Delete Room"
                      className="p-2.5 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 border border-slate-800 hover:border-rose-500/30 rounded-xl transition cursor-pointer"
                    >
          ...
          <div className="p-12 text-center bg-slate-900/40 border border-slate-800/80 rounded-3xl space-y-4">
            ...
            <p className="text-xs text-slate-400 max-w-sm mx-auto">
              No rooms matched your search criteria. Launch a new room to start collaborating!
            </p>
            <button
              onClick={() => setIsCreateModalOpen(true)}
              className="inline-flex items-center space-x-2 px-5 py-2.5 bg-emerald-400 hover:bg-emerald-300 text-slate-950 font-bold rounded-xl text-xs transition cursor-pointer shadow-lg shadow-emerald-500/20"
            >
              <Plus className="w-4 h-4 stroke-[2.5]" />
